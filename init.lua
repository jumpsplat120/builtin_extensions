...
---Formats a string similar to the C# `String.Format` method. However,
---it's only inspired by, and does not follow the same rules. When you
---have a string that contains bracket wrapped numbers (1-indexed), they are replaced
---by the variable of that index, after [tostring](command:extension.lua.doc?["en-us/51/manual.html/pdf-tostring"])'ing the
---value. Bracketed numbers with an exclamation point will have the exclamation point
---removed, but will not replace the value with text. Excess variables will be ignored,
---and if there are not enough variables for how many replacements there are, they will be
---replaced with empty strings. ex.
---```lua
---local greeting, information, mismatch
---
---greeting    = "Hello {1}! It's very nice to meet you and {2}. {2}, have you met {1} before?"
---information = "To replace a users name, simply use {!1} inside your message."
---mismatch    = "Only the first three values will be replaced. '{1}' '{2}' '{3}' '{4}' '{5}'."
---
---print(greeting:fformat("Joe", "Bob", "Cathy"))
-----outputs "Hello Joe! It's very nice to meet you and Bob. Bob, have you met Joe before?"
---
---print(information:fformat("discarded"))
-----outputs "To replace a users name, simply use {1} inside your message."
---
---print(mismatch:fformat("Hey", nil, "John"))
-----outputs "Only the first three values will be replaced. 'Hey' 'nil' 'John' '' ''."
---```
---`nil` values that are explicitly defined will be included, while variables that don't have 
---values or don't exist will also count as `nil`. So in the following cases, `fformat` will 
---function as follows:
---```lua
---local a, b, c, tbl
---a = "hello"
...
---tbl = {}
---
---print(("{1} {2} {3}"):fformat(a, b, c)) --c has been declared but not assigned a value. Parsed as nil.
-----outputs "hello world nil"
---
---print(("{1} {2}"):fformat(a, nil)) --An explicit nil value has been passed, so it's parsed as nil.
-----outputs "hello nil"
---
---print(("{1} {2}"):fformat(a, d)) --Even though d hasn't been declared, it parsed as nil.
-----outputs "hello nil"
---
---print(("{1} {2}"):fformat(a, tbl.foo)) --tbl has no key called foo, parsed as nil.
-----outputs "hello nil"
---
---print(("{1} {2}"):fformat(a)) --Only in the case where literally nothing is passed, will it parse as an empty string.
-----outputs "hello "
---```
---@param self string The string to format, with the bracketed replacement pattern.
---@param ... any The variables you want to use to replace the bracketed values with.
---@return string
---@nodiscard
...
---Returns whether a string ends with another string. This will escape
